...
def lineDrawThread(dexarm,p1,p2,wait=True):
    if(wait==True):
        lineDraw(dexarm,p1,p2)
    else:
        threadDraw=threading.Thread(target=lineDraw,args=(dexarm,p1,p2))
        threadDraw.start() 

def XDraw(dexarm:Dexarm,loc):
    global startTime
    ...
    # dexarm.move_to(None, None, lift,feedrate=robotFeedrate)
    # startTime = int(now_ns / 1000000) #Time in Milliseconds

def ODraw(dexarm:Dexarm,loc):
    ...
    while(theta <= 2*math.pi):
        dexarm.move_to(R*math.cos(theta) + xc, R*math.sin(theta) + yc, press,feedrate=robotFeedrate);
...
while True:
    ...
    # board lines
    #  vertical lines
    lineDrawThread(Xdexarm, [AB[0][0][0], AB[0][1][0]], [AB[0][0][1], AB[0][1][1]])
    lineDrawThread(Odexarm, [AB[0][0][0], AB[0][1][0]], [AB[0][0][1], AB[0][1][1]])

    #  horizontal
    lineDrawThread(Xdexarm, [AB[1][0][0], AB[1][1][0]], [AB[1][0][1], AB[1][1][1]],wait=False)
    lineDrawThread(Odexarm, [AB[1][0][0], AB[1][1][0]], [AB[1][0][1], AB[1][1][1]])

    Xdexarm.move_to(relax[0], relax[1], relax[2],feedrate=robotFeedrate,wait=False)
    Odexarm.move_to(relax[0], relax[1], relax[2],feedrate=robotFeedrate)
    ...
    for i in range(5):
        ...
        XDraw(Xdexarm,loc)
        # now_ns = time.time_ns() # Time in nanoseconds
        # nowTime = int(now_ns / 1000000) #Time in Milliseconds
        # print(nowTime-startTime);
        print("Finished Drawing X")
        ...
        print("Finished Drawing O")

        Odexarm.move_to(relax[0], relax[1], relax[2],feedrate=robotFeedrate,wait=False)
